...
const merge = (...mutations) => {
    ...
    while (mutations.length) {
        ...
        for (const key in mutation) {
            ...
            if (isMutationNull)
                doAssign();
            else if (isInstanceObject && isMutationObject)
                doMerge();
            else if (isInstanceObject && isMutationArray)
                doAssign();
            else if (isInstanceObject && isMutationValue)
                doAssign();
            else if (isInstanceArray && isMutationObject)
                doAssign();
            else if (isInstanceArray && isMutationArray)
                doClone();
            else if (isInstanceArray && isMutationValue)
                doAssign();
            else if (isInstanceValue && isMutationObject)
                doAssign();
            else if (isInstanceValue && isMutationArray)
                doAssign();
            else if (isInstanceValue && isMutationValue)
                doAssign();
        }
    }
    return instance;
